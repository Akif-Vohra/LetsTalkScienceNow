...
| **Individual rivers**<br>`features/*-river.geojson` (Godavari, Krishna, Kaveri, Narmada, Tapi, Ganga, Brahmaputra) | **Source:** the `rivers.geojson` layer above.<br>**Transformation:** each river's feature filtered out of `rivers.geojson` by its `rivname` into its own file (drawn as a highlighted course when its pin is clicked).<br>**Credit:** © **GSI** (Bhukosh). |
